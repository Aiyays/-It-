...
namespace YCF_Server.DAL
{
	...
	public partial class Nation
	{
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("NID", "Nation"); 
		}
		...
		public int Add(YCF_Server.Model.Nation model)
		{
			...
			parameters[0].Value = model.nation;
		...
		public YCF_Server.Model.Nation GetModel(int NID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 NID,Nation from Nation ");
			strSql.Append(" where NID=@NID");
			SqlParameter[] parameters = {
					new SqlParameter("@NID", SqlDbType.Int,4)
			};
			...
			YCF_Server.Model.Nation model=new YCF_Server.Model.Nation();
		...
		/// 得到一个对象实体
		/// </summary>
		public YCF_Server.Model.Nation DataRowToModel(DataRow row)
		{
			YCF_Server.Model.Nation model=new YCF_Server.Model.Nation();
			if (row != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select NID,Nation ");
